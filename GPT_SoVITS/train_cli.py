...
def audio_recognition(input_wav_path: str) -> tuple[str, Union[dict[Any, Any], dict[str, Union[str, Any]]]]:
    ...
    for i_part in range(n_parts):
        filelist = slice_audio.slice(input_wav_path, os.path.join(work_dir, "slicer_opt"), i_part, n_parts)
        for idx, file in enumerate(filelist):
            if file.endswith(".wav"):
                ...
                # TODO
                if idx == 2:
                    reference_content = {"wav_path": file, "text": recognition_text, "language": language}

    return output_slicer_path, reference_content
...
if __name__ == '__main__':
    ...
    # Start ASR
    ###############
    slicer_path, reference = audio_recognition(denoised_wav_file)
    ...
    with open(os.path.join(work_dir, "train.list"), "w", encoding="utf-8") as f:
