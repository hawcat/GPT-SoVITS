...
def slice(inp, opt_root, i_part, all_part, threshold=-34, min_length=4000, min_interval=300, hop_size=10,
          max_sil_kept=500, _max=0.9, alpha=0.25) -> list:
    ...
    for inp_path in input[int(i_part)::int(all_part)]:
        ...
        try:
            ...
            for chunk, start, end in slicer.slice(audio):  # start和end是帧数
                ...
                file_list.append("%s/%s_%s_%s.wav" % (opt_root, name, start, end))
        except:
